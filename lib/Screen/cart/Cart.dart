...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  List<Model> deliverableList = []; //13 usage
  ...
  bool _isNetworkAvailable = true; //61
  TextEditingController promoCodeController = TextEditingController(); //6
  final List<TextEditingController> _controller = []; //12

  final GlobalKey<RefreshIndicatorState> _refreshIndicatorKey = //2
      GlobalKey<RefreshIndicatorState>();
  String? msg; //9
  bool _isLoading = true; //3
  Razorpay? _razorpay; //7

  TextEditingController noteC = TextEditingController(); //2
  StateSetter? checkoutState; //39
  final paystackPlugin = PaystackPlugin(); //2
  bool deliverable = false; //10
  bool saveLater = false, //5
      addCart = false; //5
  final ScrollController _scrollControllerOnCartItems = ScrollController(); //3
  final ScrollController _scrollControllerOnSaveForLaterItems =
      ScrollController();
  TextEditingController emailController = TextEditingController(); //4
  List<String> productIds = []; //8
  List<String> proVarIds = []; //10
  var db = DatabaseHelper();
  final GlobalKey<FormState> _formkey = GlobalKey<FormState>(); //2

  bool isAvailable = true; //5
  bool confDia = true; //3

  //rozarpay
  String razorpayOrderId = ''; //4
  String? rozorpayMsg; //2
                      ...
                      child: Padding(
                          ...
                          child: Stack(
                            children: [
                              ...
                              offPer != null
                                  ? getDiscountLabel(cartList[index]
                                              .productList![0]
                                              .isSalesOn ==
                                          "1"
                                      ? double.parse(cartList[index]
...
//2
  Future<void> _getOfflineCart() async {
    if (productIds.isNotEmpty) {
      ...
      if (_isNetworkAvailable) {
        try {
          var parameter = {"product_variant_ids": productIds.join(',')};
...
//7
  ...
  razorpayPayment(String orderID, String? msg) async {
    ...
    try {
      ...
      print("rezorpay response-->${response.toString()}");
      var options = {
        KEY: razorpayId,
        AMOUNT: amt,
